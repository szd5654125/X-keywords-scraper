...
func (s *Scraper) RequestAPI(req *http.Request, target interface{}) error {
	...
	if len(s.cookie) > 0 {
		s.client.Jar.SetCookies(req.URL, s.cookie)
	} else {
		if s.isStoreCookie == false {
			s.SetCookieStore(s.client.Jar.Cookies(req.URL))
			s.isStoreCookie = true
		}
	}
